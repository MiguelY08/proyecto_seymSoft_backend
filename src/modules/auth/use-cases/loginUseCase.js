...
export class LoginUseCase {
  static async execute({ email, pass_word }) {
    // Buscar usuario
    ...
    if (!ALLOWED_LOGIN_STATUSES.includes(user.id_status)) {
    ...
    // Validar contraseña
    const isPasswordValid = await comparePassword(pass_word, user.pass_word);
    ...
    return {
      ...
      permissions: userWithRole.permissions.map((permission) => ({
