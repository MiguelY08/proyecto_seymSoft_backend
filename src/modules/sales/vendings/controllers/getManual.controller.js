...
export const GetManualVendingsController = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message:
        "Ventas manuales recuperadas exitosamente.",
      type,
      data:
        sales,
      pagination: {
    ...
    console.error(
      "[GetManualVendingsController] Error:",
      error
    );
    ...
    return res.status(500).json({
      success: false,
      message:
        "Error recuperando las ventas manuales.",
      error:
        error.message,
    });
  }
